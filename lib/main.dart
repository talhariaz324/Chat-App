/* Here after doing all things for firebase as we did in earlier module we also have to configure the firebase sdk with flutter app
Main reason is that we are not just using web apis for firebase but also going to use firebase other features and packages */

import 'package:chat_app_udemy_a/screens/auth_screen.dart';
import 'package:chat_app_udemy_a/screens/chat_screen.dart';
    ...
    return MaterialApp(
      ...
      home: StreamBuilder(
          stream: FirebaseAuth.instance
              .authStateChanges(), // It will listen to the auth state changes (handle everything itself) and will show the user screen if user is logged in or auth screen if user is not logged in
...
/* 
                                              <Push Notifications>
                                              VIDEO LINK: https://www.youtube.com/watch?v=TXod1rLVF7M&t=95s
                                              Channel ID: pushnotificationapp
IOS INTEGRATION: Check according to new update.
NOTE: By integrating firebase with flutter we can use the cloud messaging but need some improvements.
